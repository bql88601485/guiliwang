...
@synthesize token_time=_token_time;
@synthesize paied=_paied;
#pragma mark - Public
...
+ (instancetype)shareInstance 
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        sharedClient.token_time=0;
        sharedClient.paied=NO;
...
- (void)payProduct
{
   if (self.paied==NO) {
    ...
    time_t  now;
    time(&now);
    if ( (now - self.token_time) > 0 )//非测试帐号调试请启用该条件判断
    {
        //获取Token
        [self getAccessToken];//获取toke之后完成[self getPrepayId:self.accessToken];
        //设置Token有效期为10分钟
        self.token_time              = now + 6000;
        //日志输出
        //NSLog(@"获取Token： %@\n",[req getDebugifo]);
    }
    else {
    [self getPrepayId:self.accessToken];
    }
  }
}
...
{
    // 构造参数列表
    float f = _order.order_amount.floatValue * 100;
    NSString *price = [NSString stringWithFormat:@"%.0f", f];
    NSMutableDictionary *params = [NSMutableDictionary dictionary]; 
    [params setObject:@"WX" forKey:@"bank_type"];
    [params setObject:_order.subject forKey:@"body"];
    ...
    [params setObject:_order.order_sn forKey:@"out_trade_no"];
    [params setObject:[[NSUserDefaults standardUserDefaults] objectForKey:@"K_WXPARTNER_ID"] forKey:@"partner"];
    [params setObject:[CommonUtil getIPAddress:YES] forKey:@"spbill_create_ip"]; 
    [params setObject:price forKey:@"total_fee"];    // 1 =＝ ¥0.01
    ...
    [package appendString:@"key="];
    [package appendString:[[NSUserDefaults standardUserDefaults] objectForKey:@"K_WXPARTNER_KEY"]]; // 注意:不能hardcode在客户端,建议genPackage这个过程都由服务器端完成
...
- (NSMutableData *)getProductArgs
{
    ...
    [params setObject:[[NSUserDefaults standardUserDefaults] objectForKey:@"K_WX_APPID"] forKey:@"appid"];
    [params setObject:[[NSUserDefaults standardUserDefaults] objectForKey:@"K_PAY_SIGN_KEY"] forKey:@"appkey"];
...
- (void)getAccessToken
{
    NSString *getAccessTokenUrl = [NSString stringWithFormat:@"https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=%@&secret=%@", [[NSUserDefaults standardUserDefaults] objectForKey:@"K_WX_APPID"], [[NSUserDefaults standardUserDefaults] objectForKey:@"K_WX_KEY"]];
...
- (void)getPrepayId:(NSString *)accessToken
{
    ...
    [self.request setCompletionBlock:^{
        ...
        NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:[weakRequest responseData] 
                                                             options:kNilOptions 
                                                             error:&error];
        weakSelf.paied=NO;//本次支付完成
        weakSelf.token_time=0;
        if (error) {
        ...
        if (prePayId) {
            ...
            request.partnerId = [[NSUserDefaults standardUserDefaults] objectForKey:@"K_WXPARTNER_ID"];
