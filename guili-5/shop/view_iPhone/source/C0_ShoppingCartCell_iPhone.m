...
- (void)dataDidChanged
{
    ...
    if ([goods.goods_price isEqualToString:goods.old_goods_price]) {
        $(_cartCell).FIND(@"#old-subprice").HIDE();
        $(_cartCell).FIND(@"#old-subprice-line").HIDE();
    }else
    {
        $(_cartCell).FIND(@"#old-subprice").SHOW();
        $(_cartCell).FIND(@"#old-subprice-line").SHOW();
    }
    NSString *old_goods_price_str = [goods.old_goods_price stringByReplacingOccurrencesOfString :@"￥" withString:@""];
    double old_goods_price=old_goods_price_str.doubleValue*goods.goods_number.integerValue;
    if (old_goods_price>0) {
        old_goods_price_str=[[NSString alloc] initWithFormat:@"￥%.2f",old_goods_price];
        $(_cartCell).FIND(@"#old-subprice").TEXT(old_goods_price_str);
    }
