...
ON_CREATE_VIEWS( signal )
{
    ...
    [self setupToolbar];
    [self updateUI];
    [self observeNotification:BeeUIWebView.PAY_BACK];
}
...
ON_LEFT_BUTTON_TOUCHED( signal )
{
	if ( self.backBoard )
	{
        ...
        E1_PendingPaymentBoard_iPhone *uiBoard=[E1_PendingPaymentBoard_iPhone board];
        uiBoard.isPayBack=YES;
        [self.stack pushBoard:uiBoard animated:YES];
	}
	else
	{
...
ON_NOTIFICATION3( BeeUIWebView, PAY_BACK, notification )
{
    if ( self.backBoard )
    {
        // [self.stack popToBoard:self.backBoard animated:YES];
