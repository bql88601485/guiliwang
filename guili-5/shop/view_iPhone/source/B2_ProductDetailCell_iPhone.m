...
- (void)dataDidChanged
{
    ...
    if ( goodsModel )
    {
        ...
        //如果等于零隐藏
        if (goodsModel.goods.market_price !=nil) {
            NSString *market_price = [goodsModel.goods.market_price stringByReplacingOccurrencesOfString :@"￥" withString:@""];
        ...
        if (goodsModel.goods.is_shipping.boolValue) {
            $(@"#is_shipping").TEXT(@"包邮");
        }else {
             $(@"#is_shipping").TEXT(@"不包邮");
        }
        $(@"#good_number").TEXT(goodsModel.goods.goods_number);
    }
}
