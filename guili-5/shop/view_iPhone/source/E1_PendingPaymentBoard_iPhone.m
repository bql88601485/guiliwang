...
ON_SIGNAL3( E1_PendingPaymentCell_iPhone, ORDER_PAY, signal )
{
	...
	{
        if (NSOrderedSame == [order.order_info.pay_code compare:@"alipay_wap" options:NSCaseInsensitiveSearch] )
        {
            // 选择WAP支付
            ...
            [self sendUISignal:self.PAY_WAP withObject:order];
        }
        else if (NSOrderedSame == [order.order_info.pay_code compare:@"alipay_mobile" options:NSCaseInsensitiveSearch] )
        {
            // SDK支付
            /*BeeUIActionSheet * paySelect = [BeeUIActionSheet spawn];
             [paySelect addButtonTitle:@"支付宝支付" signal:self.PAY_SDK];
