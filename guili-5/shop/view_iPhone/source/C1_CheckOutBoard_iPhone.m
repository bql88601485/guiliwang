...
ON_LEFT_BUTTON_TOUCHED( signal )
{
    ...
    [self.stack popToBoard:self.shopingCartBod animated:YES];
}
...
ON_SIGNAL3( C1_CheckOutBoard_iPhone, ACTION_PAY, signal )
{
	...
	if ( order_info )
	{
        if (NSOrderedSame == [order_info.pay_code compare:@"alipay_wap" options:NSCaseInsensitiveSearch] )
        {
            // 选择WAP支付
            ...
            [self sendUISignal:self.PAY_WAP];
        }
       else if (NSOrderedSame == [order_info.pay_code compare:@"alipay_mobile" options:NSCaseInsensitiveSearch] )
		{
			// SDK支付
			/*BeeUIActionSheet * paySelect = [BeeUIActionSheet spawn];
			[paySelect addButtonTitle:@"支付宝支付" signal:self.PAY_SDK];
		...
		{
			[self.shopingCartBod presentMessageTips:__TEXT(@"pay_noneed")];
			//[self.stack popBoardAnimated:YES];
            [self.stack popToBoard:self.shopingCartBod animated:YES];
		}
