...
- (void)FogetWithUser:(NSString *)phone
             password:(NSString *)password
{
    self.CANCEL_MSG( API.user_updatepwd );
    self
    .MSG( API.user_updatepwd )
    .INPUT( @"phone", phone )
    .INPUT( @"password", password );
}
- (void)getCodeNum:(NSString *)phoneNum
{
    self.CANCEL_MSG ( API.getCode );
    self.MSG( API.getCode ).INPUT( @"phone", phoneNum);
}
- (void)user_sendcode:(NSString *)phoneNum codeNum:(NSString *)codeNum
{
    self.CANCEL_MSG( API.sendCode );
    self.MSG( API.sendCode ).INPUT( @"phone", phoneNum).INPUT( @"code", codeNum);
}
- (void)OtherUpApp:(NSString *)type username:(NSString *)username openid:(NSString *)openid
{
    self.CANCEL_MSG ( API.existuser );
    self.MSG( API.existuser ).INPUT( @"type", type).INPUT( @"username", username).INPUT( @"openid", openid );
}
- (void)signout
{
	self.CANCEL_MSG( API.user_signin );
