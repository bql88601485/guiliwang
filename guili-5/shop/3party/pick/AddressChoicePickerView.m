...
- (instancetype)init{
    ...
    if (self = [super init]) {
        ...
        NSString *path = [[NSBundle mainBundle] pathForResource:@"AreaPlist.plist" ofType:nil];
        
        self.regionArr = [[NSArray alloc] initWithContentsOfFile:path];
        ...
        self.locate.regionID = [NSNumber numberWithInt:1];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    switch (component) {
        ...
        case 3:{
            if (self.areaArr.count) {
                self.locate.area = [self.areaArr[row] objectForKey:@"name"];
                self.locate.areaID = [self.areaArr[0] objectForKey:@"id"];
            }else{
                self.locate.area = @"";
            }
