...
@synthesize old_goods_price=_old_goods_price;
...
DEF_MESSAGE_( order_pay, msg )
{
    ...
    else if ( msg.succeed )
    {
        ...
        STATUS * status 	 = [STATUS objectFromDictionary:[response dictAtPath:@"status"]];
        //NSDictionary * data  = [response dictAtPath:@"data"];
        NSString * paywap 	 = [response stringAtPath:@"data"];
...
DEF_MESSAGE_( user_signin, msg )
{
    if ( msg.sending )
    {
        ...
        //NSString *qrcodedata=[[BeeFileCache sharedInstance] objectForKey:@"qrcodedata"];
        NSData *qrcode=[[BeeFileCache sharedInstance] objectForKey:@"qrcodedata"];
        NSString *qrcodedata=@"";
        if (qrcode!=nil) {
            qrcodedata=[[ NSString alloc] initWithData:qrcode encoding:NSUTF8StringEncoding];
...
#pragma mark - POST GETCODE

DEF_MESSAGE2(getCode, msg )
{
    if (msg.sending) {
        
        NSString *phone = msg.GET_INPUT ( @"phone" );
        if (nil == phone || NO == [phone isKindOfClass:[NSString class]]) {
            msg.failed = YES;
            return;
        }
        ...
        NSString * requestURI = [NSString stringWithFormat:@"%@/user/getcode", [ServerConfig sharedInstance].url];
        ...
        STATUS * status = [STATUS objectFromDictionary:[response dictAtPath:@"status"]];
        PhoneCode * phoneCode = [PhoneCode objectFromDictionary:[response dictAtPath:@"data"]];
        ...
        if ( nil == phoneCode || NO == [phoneCode isKindOfClass:[PhoneCode class]] )
        {
            msg.failed = YES;
            return;
        }
        ...
        msg.OUTPUT( @"data_phoneCode", phoneCode );
...
#pragma mark - POST sendCode

DEF_MESSAGE2(sendCode , msg ){
    ...
    if (msg.sending) {
        ...
        if (nil == code || NO == [code isKindOfClass:[NSString class]]) {
            msg.failed = YES;
            return;
        }
        NSData *qrcode=[[BeeFileCache sharedInstance] objectForKey:@"qrcodedata"];
        ...
        NSString * requestURI = [NSString stringWithFormat:@"%@/user/sendcode", [ServerConfig sharedInstance].url];
        ...
        SESSION * session = [SESSION objectFromDictionary:[response dictAtPath:@"data.session"]];
        STATUS * status = [STATUS objectFromDictionary:[response dictAtPath:@"status"]];
        NSDictionary *dic = [response dictAtPath:@"data"];
...
#pragma mark - POST existuser
DEF_MESSAGE2( existuser, msg){
...
#pragma mark - POST user/updatepwd

DEF_MESSAGE_( user_updatepwd, msg )
{
    if ( msg.sending )
    {
        NSString * phone = msg.GET_INPUT( @"phone" );
        ...
        NSString * requestURI = [NSString stringWithFormat:@"%@/user/updatepwd", [ServerConfig sharedInstance].url];
...
@synthesize url = _url;
@synthesize app_root_url=_app_root_url;
@synthesize root_url=_root_url;
@end
